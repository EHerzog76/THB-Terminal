 ...
 *
 * $Id: ProtocolComponent.cs,v 1.1 2010/11/19 15:41:03 kzmi Exp $
 */
using System;
...
namespace Protocols
{
    internal class MonitorableStreamTemplate<T> {
        ...
        public void AddMonitor(T monitor) {
            if (_monitors == null) _monitors = new LinkedList<T>();
            _monitors.AddLast(monitor);
        }
        public void RemoveMonitor(T monitor) {
            if(_monitors!=null) _monitors.Remove(monitor);
        }
        ...
        protected void ResetEnumerator() {
            if (_enumerator == null)
                ...
                _enumerator.Reset(); //TODO ƒRƒŒƒNƒVƒ‡ƒ“‚Ì’†g•Ï‚í‚Á‚Ä‚àReset‚ª—LŒø‚È‚±‚Æ‚ðŠm”F
        }
    }
	...
	internal class MonitorableByteAsyncInputStream : MonitorableStreamTemplate<IByteAsyncInputStream>, IByteAsyncInputStream {
        ...
        public void OnReception(ByteDataFragment data) {
            if (_monitors != null) {
                ResetEnumerator();
                while (_enumerator.MoveNext()) _enumerator.Current.OnReception(data);
            }
        ...
        public void OnNormalTermination() {
            if (_monitors != null) {
                ResetEnumerator();
                while (_enumerator.MoveNext()) _enumerator.Current.OnNormalTermination();
            }
        ...
        public void OnAbnormalTermination(string message) {
            if (_monitors != null) {
                ResetEnumerator();
                while (_enumerator.MoveNext()) _enumerator.Current.OnAbnormalTermination(message);
            }
    ...
    internal class MonitorableByteOutputStream : MonitorableStreamTemplate<IByteOutputStream>, IByteOutputStream {
        ...
        public void Transmit(string data)
        {
            ...
            {
                ResetEnumerator();
                while (_enumerator.MoveNext()) _enumerator.Current.Transmit(data);
            }
        ...
        public void Transmit(byte[] buffer, int offset, int length) {
            if (_monitors != null) {
                ResetEnumerator();
                while (_enumerator.MoveNext()) _enumerator.Current.Transmit(buffer, offset, length);
            }
        ...
        public void Close() {
            if (_monitors != null) {
                ResetEnumerator();
                while (_enumerator.MoveNext()) _enumerator.Current.Close();
            }
            _transmitter.Close();
        }
