...
namespace TerminalEmulator
{
	...
	public class ISO2022CharDecoder : ICharDecoder {
		...
		private class DECLineByteProcessor : IByteProcessor {
			...
			static DECLineByteProcessor() {
                ResourceManager m = new ResourceManager("TerminalEmulator.rulechars", Assembly.GetAssembly(typeof(TerminalEmulatorPlugin)));    // typeof(GEnv).Assembly
				DEC_SPECIAL_LINES = m.GetString("DECLineChars").ToCharArray();
				//ƒŠƒ\[ƒX‚Å?‚É‚È‚Á‚Ä‚¢‚½‚Æ‚±‚ë‚ð–„‚ß‚é
				DEC_SPECIAL_LINES[2] = (char)0x09;
				DEC_SPECIAL_LINES[3] = (char)0x0C;
				DEC_SPECIAL_LINES[4] = (char)0x0D;
				DEC_SPECIAL_LINES[5] = (char)0x0A;
				DEC_SPECIAL_LINES[8] = (char)0x0A; //NL‚Æ‚ ‚é‚ª‚æ‚­‚í‚©‚ç‚È‚¢BLF‚É‚µ‚Ä‚¨‚­B
            ...
            */
			...
			public void ProcessByte(byte b) {
				if (0x60 <= b && b <= 0x7F) {
                    ...
                    char linechar = DEC_SPECIAL_CHARACTERS[b - 0x60];
        ...
        //Central Entry for Char-Processing
        // Remove ANSI-Chars, VT100, ...
				...
				if(_state==State.Normal && !IsControlChar(b) && _encoding.IsInterestingByte(b)) {
					...
					switch(_state) {
						...
						case State.ESC_BRACKET:
							...
							else if(b==(byte)'B' || b==(byte)'J' || b==(byte)'~') { //!!less‚Åssh2architecture.txt‚ðŒ©‚Ä‚¢‚½‚ç—ˆ‚½BÚ×‚Í‚Ü‚¾’²‚×‚Ä‚¢‚È‚¢B
								_G0ByteProcessor = _asciiByteProcessor;
								ChangeProcessor(_G0ByteProcessor);
								_state = State.Normal;
							}
